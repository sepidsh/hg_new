...
class FloorplanGraphDataset(Dataset):
	...
	def __getitem__(self, index):
		...
		# build input graph"""
		rooms_bbs, graph_nodes, graph_edges = self.build_graph(rms_bbs, rms_type_new, fp_eds, eds_to_rms)
		rooms_mks = self.draw_masks(rms_type, fp_eds, eds_to_rms_tmp,rms_no)
	...
	def draw_masks(self, rms_type, fp_eds, eds_to_rms,rm_no, im_size=512):
		import math
		# import webcolors
		# full_img = Image.new("RGBA", (256, 256), (255, 255, 255, 0))  # Semitransparent background.
		rms_masks = []
		fp_mk = np.zeros((128, 128))
		for k in range(len(rms_type)):
			...
			rm_arr = np.array(rm_im.resize((128, 128)))
			...
			for i in range(len(ind_x)):
				...
				#print("ind",ind)
				#print("inds",inds)
				if(fp_mk[ind_x[i]][ind_y[i]]<=rm_no) :

					fp_mk[ind_x[i]][ind_y[i]] = k+1
		...
		for k in range(len(rms_type)):
				...
				rm_arr[inds] = 1.0
				#img = Image.fromarray(rm_arr, 'RGB')
				#rm_arr1=np.array(img.resize((32,32)))
