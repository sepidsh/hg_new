...
checkpoint = '/home/nelson/Workspace/autodesk/housegan2/checkpoints/exp_high_res_with_doors_64x64_A_250000.pth'
...
def draw_graph(g_true):
    ...
    nx.draw(G_true, pos, node_size=node_size, node_color=colors_H, font_size=14, font_color='white', font_weight='bold', edges=edges, edge_color=colors, width=weights, with_labels=True)
    plt.tight_layout()
...
def detailed_viz(masks, nodes, G_gt):
    ...
    nx.draw(G_estimated_complete, pos, node_size=node_size, node_color=colors_H, font_size=14, font_weight='bold', font_color='white', \
            edges=G_estimated_complete.edges(), edge_color=colors, width=weights, with_labels=True)
    plt.tight_layout()
    plt.savefig('./dump/_fake_graph.jpg', format="jpg")
...
for i, sample in enumerate(fp_loader):
    ...
    all_rooms = [i for i in range(real_nodes.shape[0]) if real_nodes[i]+1 != 15 and real_nodes[i]+1 != 17]
    all_nodes = list(range(nds.shape[0]))
    ...
    for _ in range(20):
        ...
        for _t in room_types:
            prev_fixed_nodes_state = list(np.where(real_nodes < _t)[0])
            curr_fixed_nodes_state = list(np.where(real_nodes <= _t)[0])
    ...
    save_image(all_images, '{}/runs/exp_add_doors_random_64x64_g2.png'.format(PREFIX), nrow=6, normalize=False)
    exit(0)